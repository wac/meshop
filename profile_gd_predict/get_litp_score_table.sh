...
echo "dpc<-read.table('disease-pharma-chem-profiles-score3,4,8.txt', sep='|', header=FALSE, quote='', comment.char='')" > $TXTDIR/$SCRIPTFILE
...
echo "dpc.pscore8<-quantile(dpc[,3], seq(0,0.99, 0.01))" >> $TXTDIR/$SCRIPTFILE

echo "dpc.pscores<-cbind(dpc.pscore3, dpc.pscore4, dpc.pscore8)" >> $TXTDIR/$SCRIPTFILE
...
# TRY HIGHER than the score (score weirdness?!)
echo "dpc.lit_p<-cbind(probs, apply(probs, 1, function(x) sum(dpc[,1] > dpc.pscore3[x[1]] & dpc[,2] > dpc.pscore4[x[2]] & dpc[,3] > dpc.pscore8[x[3]])))" >> $TXTDIR/$SCRIPTFILE
